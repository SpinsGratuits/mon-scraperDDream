...
# --- 1. CONFIGURATION ---
# URL configurée spécifiquement pour la page Dice Dreams de Mosttechs
url = "https://mosttechs.com/free-dice-dreams-rolls/"
filename = "scrapdicedreams.json"  # Fichier dédié à Dice Dreams
...
if status_code == 200:
    ...
    for element in entry_content.find_all(["p", "ul", "ol", "strong"]):
        ...
        # Détection d'une ligne de date (Ex: "25 sep 2026" ou "25 september 2026")
        ...
        if match_date:
            ...
            # Gestion des abréviations de mois (ex: "sep" -> "september")
            if nom_mois == "sep":
                nom_mois = "september"
            elif nom_mois == "feb":
                nom_mois = "february"
        ...
        for link in links:
            ...
            # Mots-clés d'identification des redirections de récompense Dice Dreams
            ...
            if any(key in href.lower() for key in keywords):
                ...
                if href in anciens_liens:
                    # ANCIEN LIEN : On réinjecte les anciennes clés historiques sans altération
                    json_data.append({
                    ...
                    # NOUVEAU LIEN : On capture l'heure courante de sa première découverte par le robot
    ...
    # Tri décroissant basé sur l'horodatage calendrier
    ...
    print(f"[Terminé] Fichier Dice Dreams {filename} généré avec succès ({len(json_data)} liens classés chronologiquement).")
